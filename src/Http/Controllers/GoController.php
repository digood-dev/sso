...
class GoController
{
    ...
    {
        if ($request->session()->get('sso_login', false)) response('<h1>无法流转至其它子系统<h1><hr><small>当前登录态来自其它系统流转，不支持继续流转至其它子系统</small>', 500);

        $token = (new SsoService())->client()->getAccessToken();
        if (empty($token)) return response('<h1>无法提取的你联合账户TOKEN<h1><hr><small>可能的原因：</small><small>未使用SSO多谷联合账户进行登录</small><small>从其它系统流转的登录态</small>', 500);
