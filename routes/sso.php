...
Route::prefix('sso')
    ...
    ->group(function () {
        Route::get('/sign-in', [SsoController::class, 'sign_in'])->name('sso.sign-in');// 传统登录
        Route::get('/sign-in/by_wecom', [SsoController::class, 'sign_in_by_wecom'])->name('sso.sign-in.by_wecom');// 企业微信扫码登录
        Route::get('/sign-in/by_token/{token}', [SsoController::class, 'sign_in_by_token'])->name('sso.sign-in.by_token');
        Route::get('/sign-in/by_key/{key}', [SsoController::class, 'sign_in_by_key'])->name('sso.sign-in.by_key');// 通过临时Key登录
